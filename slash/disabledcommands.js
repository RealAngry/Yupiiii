...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('List all disabled commands and categories in the server')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
        .addChannelOption(option => 
            option.setName('channel')
                .setDescription('The channel to list disabled commands for (leave empty for all)')
    ...
    async execute(interaction, client) {
        ...
        // If channel is specified, filter by channel
        if (channel) {
        ...
        try {
            ...
            if (disabledItems.length === 0) {
                return interaction.editReply(`No commands or categories are disabled ${channel ? `in ${channel}` : 'in this server'}.`);
            }
            ...
            return interaction.editReply('An error occurred while trying to list disabled commands.');
        }
    }
}; 